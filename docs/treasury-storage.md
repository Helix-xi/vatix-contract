...
| `PendingAdmin` | `instance` | `PendingAddressChange` | Nominated admin awaiting the `ADDRESS_TIMELOCK_SECONDS` (48h) delay before `execute_admin` can install it. |
| `AuthorizedMarkets` | `instance` | `Vec<Address>` | The set of market contract addresses allowed to call `collect_fee`. Managed via `add_market` / `remove_market` (instant, admin-only) and the timelocked `propose_market_contract` / `execute_market_contract` (append-only as of #720 — see that fix's notes for the registry-wipe bug it closed). Returns an empty list when unset (not an error). |
| `PendingMarketContract` | `instance` | `PendingAddressChange` | Market contract address proposed via `propose_market_contract`, awaiting the timelock before `execute_market_contract` appends it to `AuthorizedMarkets`. |
...
| `Stakeholders` | `instance` | `Vec<(Address, u32)>` | Ordered list of `(stakeholder_address, share_bps)` pairs used by `distribute_fees` (#485). All `share_bps` values must sum to exactly `10_000`, enforced at `propose_stakeholders` time (#721). Empty list when no stakeholder change has ever executed — `distribute_fees` rejects with `NoStakeholdersConfigured` in that case. |
| `PendingStakeholders` | `instance` | `PendingStakeholders` | Stakeholder list proposed via `propose_stakeholders` (Issue #689), awaiting the timelock before `execute_stakeholders` installs it. |
